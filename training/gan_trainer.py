...
class GanTrainer:
    def __init__(self,
        ...
        self.log_root = Path(log_root).resolve() / generator_name / timestamp
        self.model_root = Path(model_root).resolve() / generator_name / timestamp
        self.summary_writer = SummaryWriter(str(self.log_root / "tensorboard"))

        self.metrics_score = metrics_score or default_metric_score

        self.model_name = generator_name
